...
/**
 * adminController
 * - ใช้กับหน้า: แอดมินแดชบอร์ด (ผู้ใช้ / ประมูล / เติมเงิน / ถอน / สถิติ)
 * - mount จริงที่: `/api/admin` (`routes/adminRoutes.js`)
 * - ทุก route ต้อง JWT + role admin; อนุมัติถอนใช้ multipart field `slip`
 */
// บันทึก audit log การกระทำของแอดมิน (ไม่ใช่ HTTP handler)
...
// GET /api/admin/users — รายชื่อผู้ใช้ (id, username, role, balance)
...
// GET /api/admin/users/:id/profile — โปรไฟล์เต็มของ user (สำหรับ modal ในแอดมิน)
...
// GET /api/admin/users/:id/activity — timeline กิจกรรม (เติม/ถอน/ประมูล/bid)
...
// GET /api/admin/stats — สรุปตัวเลขระบบ + top-up / transaction ล่าสุด
...
// GET /api/admin/auctions — รายการประมูลสำหรับแอดมิน (พร้อมผู้ขาย/ผู้ชนะ)
...
// PATCH /api/admin/auctions/:id — แก้ฟิลด์ประมูล (แอดมิน)
export async function patchAuction(req, res) {
  const pool = await getPool();
...
// POST /api/admin/auctions/:id/cancel — บังคับจบประมูลทันที (ตั้ง end_time = now)
...
// GET /api/admin/top-ups — รายการคำขอเติมเงิน + filter/pagination
export async function listTopUps(req, res) {
  const pool = await getPool();
...
// GET /api/admin/top-ups/:id/logs — audit log ของคำขอเติมเงิน
export async function getTopUpLogs(req, res) {
  const pool = await getPool();
  try {
    const [rows] = await pool.query(
...
// GET /api/admin/withdrawals — รายการคำขอถอน + filter/pagination
...
// GET /api/admin/withdrawals/:id/logs — audit log ของคำขอถอน
export async function getWithdrawalLogs(req, res) {
  const pool = await getPool();
  try {
    const [rows] = await pool.query(
      `SELECT l.*, u.username AS actor_username
...
// POST /api/admin/withdrawals/:id/approve — อนุมัติถอน (แนบสลิป `slip` + แจ้ง user)
...
// POST /api/admin/withdrawals/:id/reject — ปฏิเสธถอน + คืน balance ให้ user
...
// POST /api/admin/top-ups/:id/approve — อนุมัติเติมเงิน + เพิ่ม balance
...
// POST /api/admin/top-ups/:id/reject — ปฏิเสธเติมเงิน (ไม่เพิ่ม balance)
export async function rejectTopUp(req, res) {
  const pool = await getPool();
  const conn = await pool.getConnection();
  const { note } = req.body;
...
// PATCH /api/admin/users/:id — แก้ role หรือ balance โดยตรง
...
// DELETE /api/admin/users/:id — ลบ user + bid ของ user (ระวังข้อมูลอ้างอิง)
...
// DELETE /api/admin/auctions/:id — ลบประมูล + bids
...
// POST /api/admin/users/:id/add-funds — เพิ่มยอด balance แบบ manual (+ audit)
export async function addFunds(req, res) {
  try {
    ...
    await logAdminAction(pool, req.user.id, 'manual_add_funds', 'user', Number(req.params.id), {
...
// POST /api/admin/users/:id/deduct-funds — หักยอด balance แบบ manual (+ audit)
export async function deductFunds(req, res) {
  try {
    ...
    await pool.query('UPDATE users SET balance = balance - ? WHERE id = ?', [amount, req.params.id]);
    await logAdminAction(pool, req.user.id, 'manual_deduct_funds', 'user', Number(req.params.id), {
      amount: Number(amount),
      note: note || null
    });
