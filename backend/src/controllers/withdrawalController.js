...
// อ่านค่าธรรมเนียมถอนจาก env (ค่าเริ่มต้น 20 บาท) — ใช้เฉพาะใน controller นี้
 ...
 * - ใช้กับหน้า: ถอนเงิน / ประวัติคำขอถอน
 * - mount จริงที่: `/api/withdrawals`
 * - หมายเหตุ: สร้างคำขอแล้วหัก balance ทันที (รอ admin อนุมัติ/ปฏิเสธ)
 */
// GET /api/withdrawals/me — รายการคำขอถอนของผู้ใช้
...
// POST /api/withdrawals — สร้างคำขอถอน (ล็อกแถว user + หักยอด + แจ้งเตือน)
export async function create(req, res) {
  ...
  if (!trimmedAccount || trimmedAccount.length < 6) {
    return res.status(400).json({ message: 'กรุณากรอกเลขบัญชีให้ถูกต้อง' });
  }
