...
/**
 * paymentController
 * - ใช้กับหน้า: คำสั่งซื้อ / ชำระเงิน / จัดส่ง / ยืนยันรับของ / แจ้งเตือนเกี่ยวกับการชำระเงิน
 * - mount จริงที่: `/api/payments` (`routes/paymentRoutes.js`)
 */
...
export async function getTransactions(req, res) {
  ...
  try {
    ...
    transactions.forEach((t) => {
      console.log(
        `- "${t.auction_title}" (ID: ${t.id}) - Winner: ${t.winner_username}, Seller: ${t.seller_username}, Status: ${t.status}`
      );
    });
...
// GET /api/payments/transactions/:id — รายละเอียดธุรกรรม (ต้องเป็น party ของ order)
...
// POST /api/payments/transactions — สร้าง order หลังประมูลจบ (จาก auction_id)
export async function createTransaction(req, res) {
  ...
  try {
    ...
    const [existingTransactions] = await pool.query(`SELECT id FROM payment_transactions WHERE auction_id = ?`, [
      auction_id
    ]);
    ...
    const platformFee = auction.winning_amount * 0.05;
    ...
    `,
      [
        transactionId,
        req.user.id,
        `Payment of $${auction.winning_amount} is pending for auction "${auction.title}". Please complete the payment to proceed.`
      ]
    );
    ...
    `,
      [
        transactionId,
        auction.user_id,
        `Payment of $${auction.winning_amount} is pending for auction "${auction.title}". Waiting for winner to complete payment.`
      ]
    );
...
// POST /api/payments/transactions/:id/pay — ผู้ชนะชำระเงิน + ข้อมูลจัดส่ง (เข้า escrow)
...
// POST /api/payments/transactions/:id/ship — ผู้ขายอัปเดตสถานะ shipped + ข้อมูลขนส่ง
export async function shipTransaction(req, res) {
  ...
  try {
    ...
    `,
      [
        req.params.id,
        transaction.winner_id,
        `📦 Your item from auction "${transaction.auction_title}" has been shipped! Tracking: ${tracking_number || 'N/A'}. Please confirm delivery when you receive it.`
      ]
    );
    ...
    `,
      [
        req.params.id,
        req.user.id,
        `📦 You have shipped the item for auction "${transaction.auction_title}". Tracking: ${tracking_number || 'N/A'}. Waiting for buyer confirmation.`
      ]
    );
...
export async function deliverTransaction(req, res) {
  const pool = await getPool();
  ...
  try {
    ...
    console.log(
      `📊 Transaction exists: ID=${allTransaction.id}, Winner=${allTransaction.winner_id}, Seller=${allTransaction.seller_id}, Status=${allTransaction.status}`
    );
    ...
    if (transaction.status !== 'shipped') {
      ...
      return res
        .status(400)
        .json({ message: `Transaction is not in shipped status. Current status: ${transaction.status}` });
    }
    ...
    const [escrowRows] = await pool.query(`SELECT seller_amount FROM payment_escrow WHERE transaction_id = ?`, [
      req.params.id
    ]);
    ...
    `,
      [
        req.params.id,
        transaction.seller_id,
        `🎉 Great news! The buyer has confirmed receiving the item from auction "${transaction.auction_title}". Payment of $${sellerAmount} has been released to your account.`
      ]
    );
    ...
    `,
      [
        req.params.id,
        req.user.id,
        `✅ You have confirmed receipt of the item from auction "${transaction.auction_title}". The seller has been notified and payment has been released.`
      ]
    );
...
// POST /api/payments/transactions/:id/complete — ปิดงาน (completed) หลัง delivered
...
export async function listPaymentNotifications(req, res) {
  const pool = await getPool();
...
// PUT /api/payments/notifications/:id/read — ทำเครื่องหมายอ่านแจ้งเตือนการชำระเงิน
export async function markPaymentNotificationRead(req, res) {
  const pool = await getPool();
...
// GET /api/payments/balance — ยอด balance ปัจจุบัน (สำหรับหน้าชำระเงิน/เช็คยอด)
...
// POST /api/payments/transactions/:id/tracking — ผู้ขายอัปเดตเลขพัสดุ/วันที่คาดส่ง
