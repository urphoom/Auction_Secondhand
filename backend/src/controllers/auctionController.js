...
/**
 * auctionController
 * - ใช้กับหน้า: รายการประมูล / รายละเอียด / สร้าง-แก้ไข listing / ยื่นราคา / ซื้อทันที
 * - mount จริงที่: `/api/auctions`
 * - multer, ลำดับ path (`/active` ก่อน `/:id`) อยู่ที่ `routes/auctionRoutes.js`
 */
// แปลง `images` (JSON/string) + fallback จาก `image` ให้เป็น array เดียวกันสำหรับ API
...
// GET /api/auctions/active — ประมูลที่ยังไม่จบ (เรียงตาม end_time)
...
// GET /api/auctions/my-active-count — จำนวน listing ที่ยัง active ของผู้ใช้ที่ล็อกอิน
export async function getMyActiveCount(req, res) {
  const pool = await getPool();
...
// GET /api/auctions/my-listing-counts — นับ active vs จบแล้วของผู้ขาย
export async function getMyListingCounts(req, res) {
  const pool = await getPool();
  const userId = req.user.id;
...
// GET /api/auctions/:id/highest-bid — ราคาสูงสุด (ประมูลปิดซองจะซ่อนจนกว่าจะจบ)
export async function getHighestBid(req, res) {
  const pool = await getPool();
...
// GET /api/auctions/:id/top-bidders — top 5 ผู้บิด (sealed ก่อนจบคืน [])
export async function getTopBidders(req, res) {
  const pool = await getPool();
...
// GET /api/auctions — รายการประมูลทั้งหมด
...
// GET /api/auctions/my-auctions — ประมูลที่จบแล้วของผู้ขายที่ล็อกอิน (มีผู้ชนะ/ยอดชนะ)
...
// GET /api/auctions/my-bid-history — ประวัติการบิดของผู้ใช้ (รวมสถานะชนะ/แพ้หลังจบ)
export async function getMyBidHistory(req, res) {
  const pool = await getPool();
  const userId = req.user.id;
  const [rows] = await pool.query(
    `
...
// GET /api/auctions/:id/has-bid — เคยบิดในรายการนี้หรือยัง
export async function hasBid(req, res) {
  const pool = await getPool();
...
// POST /api/auctions/:id/finalize — เรียกจบประมูล + ส่งแจ้งเตือน (มักเรียกอัตโนมัติ/หลังหมดเวลา)
...
// GET /api/auctions/:id — รายละเอียด + bids (sealed จะไม่ส่งรายการ bids จนกว่าจะจบ)
...
// GET /api/auctions/:id/bids/owner — ประวัติ bid ฉบับเต็ม (เฉพาะเจ้าของ listing)
export async function getBidsOwner(req, res) {
  const pool = await getPool();
...
// POST /api/auctions — สร้างประมูล (multipart: `image` 1 ไฟล์ + `images` สูงสุด 5)
export async function create(req, res) {
  try {
    ...
    const minIncrement = selectedBidType === 'increment' ? Number(minimum_increment) || 1.0 : null;
...
// PUT /api/auctions/:id — แก้ title/description/end_time + รูปหลัก (optional `image`)
export async function update(req, res) {
  ...
  await pool.query('UPDATE auctions SET title=?, description=?, image=?, end_time=? WHERE id=?', [
    title ?? auction.title,
...
// PATCH /api/auctions/:id/edit — แก้ราคา/เวลา/รูปหลายใบ (ห้ามมี bid แล้ว)
...
// DELETE /api/auctions/:id — ลบ listing + bids (เจ้าของหรือ admin)
...
// POST /api/auctions/:id/bids — ยื่นราคา (หัก/คืน balance ตามประเภทประมูล)
  ...
  try {
    ...
    if (auction.bid_type === 'increment') {
      ...
      if (
        ...
        amount < Number(auction.current_price) + Number(auction.minimum_increment)
      ) {
        throw new Error(
          `Bid must be at least ${Number(auction.current_price) + Number(auction.minimum_increment)} (current price + minimum increment)`
        );
      }
...
// POST /api/auctions/:id/buy-now — ซื้อทันที (จบประมูล + แจ้งเตือน)
